    ...
    if request.method == "POST":
        ...
        if request_url == '/config/ConsultaPublica/traeListaProyectoConsultaAvanzada':
            ...
            for i in data_json['Data']:
                # print(i['Codigo'])
                if i['Marco'] == "SNIP":
                    titulo_response = requests.get(
                        'http://ofi4.mef.gob.pe/bp/ConsultarPIP/titulo.asp?donde=consulta&codigo=%s&version=1&ed=' % i[
                            'Codigo'])
                    check_doc = 'ofi4.mef.gob.pe/appFs/ListaPIP.aspx?pip=' in titulo_response.text
                    i['DocumentosDeViabiliadad'] = "<a target=\"_blank\" href=\"http://ofi4.mef.gob.pe/appFs/ListaPIP.aspx?pip=%s\">SI</a>" % \
                                                     i['Codigo'] if check_doc else "NO"
